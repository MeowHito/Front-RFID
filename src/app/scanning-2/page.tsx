...
export default function Scanning2RedirectPage() {
        ...
        (async () => {
            try {
                ...
                if (res.ok) {
                    ...
                    if (data?.slug) {
                        router.replace(`/scanning-2/${data.slug}`);
                        return;
                    }
                    if (data?._id) {
                        router.replace(`/scanning-2/${data._id}`);
                        return;
                    }
                }
                setError(true);
                ...
                {error ? (
                    <>
                        ...
                        <div style={{ fontSize: 16, color: '#94a3b8' }}>
                            กรุณาไปที่หน้า Admin Events แล้วกดดาวเลือกกิจกรรม หรือใช้ลิงก์ /scanning-2/ชื่อกิจกรรม โดยตรง
                        </div>
                    </>
                ) : (
                    <>
