        ...
        (async () => {
            try {
                ...
                if (res.ok) {
                    ...
                    if (data?.slug) {
                        router.replace(`/Overall-Winners/${data.slug}`);
                        return;
                    }
                    if (data?._id) {
                        router.replace(`/Overall-Winners/${data._id}`);
                        return;
                    }
                }
                setError(true);
            ...
            {error ? (
                <>
                    ...
                    <div style={{ fontSize: 16, color: '#94a3b8' }}>
                        กรุณาไปที่หน้า Admin Events แล้วกดดาวเลือกกิจกรรม หรือใช้ลิงก์ /Overall-Winners/ชื่อกิจกรรม โดยตรง
                    </div>
                </>
            ) : (
                <>
