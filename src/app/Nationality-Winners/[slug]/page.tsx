...
export default function NationalityWinnersBySlugPage() {
    ...
    const downloadLandscape = useCallback(async (gender: 'male' | 'female' | 'both' = 'both') => {
        ...
        try {
            ...
            triggerSingleDistanceDownload(blob, campaign.name || '', 'Nationality', selectedCategory, distance, gender);
                    ...
                    {campaign && !initialLoading && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
                            ...
                            <button
                                ...
                                title="Download Nationality Winners (Excel)"
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? 6 : '0.4vw', flexWrap: 'wrap' }}>
                            <div style={{ display: 'flex', gap: isMobile ? 6 : '0.4vw', overflowX: 'auto', WebkitOverflowScrolling: 'touch', paddingBottom: isMobile ? 2 : 0 }}>
                                {campaign.categories.map(cat => (
                                    <button
                                        ...
                                        style={{ padding: isMobile ? '6px 12px' : '0.4vh 1vw', borderRadius: 6, fontSize: isMobile ? 12 : '1.3vh', fontWeight: 700, border: selectedCategory === cat.name ? '2px solid #4f46e5' : '1px solid #475569', background: selectedCategory === cat.name ? '#4f46e5' : 'transparent', color: selectedCategory === cat.name ? '#fff' : '#cbd5e1', cursor: 'pointer', whiteSpace: 'nowrap', flexShrink: 0 }}
                                    >
                            ...
                            {campaign.categories.length > 1 && (
                                <button
                                    ...
                                    style={{ display: 'flex', alignItems: 'center', gap: 6, padding: isMobile ? '6px 12px' : '0.4vh 0.8vw', background: autoMode ? '#4f46e5' : 'transparent', border: `1px solid ${autoMode ? '#4f46e5' : '#475569'}`, borderRadius: 6, color: autoMode ? '#fff' : '#94a3b8', fontSize: isMobile ? 12 : '1.3vh', fontWeight: 800, cursor: 'pointer', whiteSpace: 'nowrap', flexShrink: 0, minWidth: isMobile ? 80 : 72, justifyContent: 'center', transition: 'background 0.2s, color 0.2s, border-color 0.2s' }}
                                >
            ...
            {campaign && (
                <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', alignItems: 'center', justifyContent: 'center', gap: isMobile ? 4 : '0.8vw', padding: isMobile ? '8px 12px' : '0.5vh 1.5vw', background: '#1e293b', borderRadius: 10, marginBottom: isMobile ? 8 : '0.8vh', border: '1px solid #334155', flexShrink: 0, textAlign: 'center' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: isMobile ? 2 : '0.2vh' }}>
                        ...
                        <span style={{ color: '#4f46e5', fontWeight: 900, fontSize: isMobile ? 11 : '1.4vh', letterSpacing: 1.5, textTransform: 'uppercase', whiteSpace: 'nowrap' }}>Nationality Winners</span>
                    </div>
                    {selectedCategory && (
                        <span style={{ display: 'inline-flex', alignItems: 'center', padding: isMobile ? '3px 14px' : '0.2vh 1.2vw', background: '#4f46e5', color: '#fff', borderRadius: 999, fontWeight: 900, fontSize: isMobile ? 13 : '1.8vh', letterSpacing: 1, whiteSpace: 'nowrap', flexShrink: 0 }}>
                            {selectedCategory}
