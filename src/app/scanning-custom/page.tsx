...
export default function ScanningCustomRedirectPage() {
        ...
        (async () => {
            try {
                ...
                if (res.ok) {
                    ...
                    if (data?._id) { router.replace(`/scanning-custom/${data._id}`); return; }
                }
                setError(true);
            } catch {
                setError(true);
                ...
                {error ? (
                    <>
                        ...
                        <div style={{ fontSize: 16, color: '#94a3b8' }}>
                            กรุณาไปที่หน้า Admin Events แล้วกดดาวเลือกกิจกรรม หรือใช้ลิงก์ /scanning-custom/ชื่อกิจกรรม โดยตรง
                        </div>
                    </>
                ) : (
                    <>
                        <div style={{
                            width: 40, height: 40, border: '4px solid #334155', borderTopColor: '#4ade80',
                            borderRadius: '50%', animation: 'spin 0.8s linear infinite',
                        }} />
                        <div style={{ fontSize: 16, color: '#94a3b8', marginTop: 20 }}>กำลังเปลี่ยนเส้นทาง...</div>
                    </>
                )}
            </div>
