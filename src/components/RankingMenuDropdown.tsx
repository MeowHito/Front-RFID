...
export default function RankingMenuDropdown({
    ...
    if (!isAdmin && !hasPublicItems) return null;
        ...
        <div ref={rootRef} className="relative shrink-0">
            <button
                ...
                className={`flex items-center whitespace-nowrap rounded-full text-xs font-bold text-[var(--muted-foreground)] ${iconOnly
                    ? 'h-[24px] w-[24px] justify-center border-none bg-transparent'
                    : 'gap-1.5 border border-[var(--border)] bg-[var(--card-solid)] px-3.5 py-1.5'}`}
            >
                <svg aria-hidden width={iconOnly ? '12' : '14'} height={iconOnly ? '12' : '14'} viewBox="0 0 24 24" fill="none" stroke="#eab308" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="shrink-0">
